...
import { createCompactModel, createProvider, selectModelTier } from "./providers/router.js";
...
import type { Session, RunOptions } from "./config/types.js";

dotenv.config();

export async function run(task: string, options: RunOptions = {}): Promise<void> {
        ...
        try {
                ...
                session.tools = new ToolRegistry(sandbox, retriever, diffManager, projectRoot, session);
