...
export async function checkMaturity() {
  ...
  let iterations = 0;
  try {
      const logPath = path.join(projectRoot, "docs", "prompt-iterations.md");
      const content = await fs.readFile(logPath, "utf-8");
      iterations = (content.match(/^\| 20/gm) || []).length;
  } catch {}
  
  const iterStatus = iterations >= 6 ? "✅" : "🚧";
  console.log(`- Prompt Optimization:   ${iterStatus} ${iterations}/6 iterations`);

  // 2. Golden Set (>= 150 tasks)
  let taskCount = 0;
  try {
      const v1 = await fs.readdir(path.join(projectRoot, "evals", "golden-set", "v1"));
      const v2 = await fs.readdir(path.join(projectRoot, "evals", "golden-set", "v2"));
  ...
  console.log(`- Training Examples:     ${exampleStatus} ${examples}/500 collected`);

  // 4. pass@1 (> 70%)
  const passRate = metrics.successRate * 100;
  const passStatus = passRate >= 70 ? "✅" : "🚧";
  console.log(`- pass@1 Success Rate:   ${passStatus} ${passRate.toFixed(1)}% (Target: >70%)`);

  console.log("──────────────────────────────────────────────────────────");
  
  const totalProgress = ((Math.min(iterations, 6) / 6) + 
                         (Math.min(taskCount, 150) / 150) + 
                         (Math.min(examples, 500) / 500) + 
                         (Math.min(passRate, 70) / 70)) / 4 * 100;
