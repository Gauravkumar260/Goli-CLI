...
describe('T-060: flickerStore', () => {
  ...
  it('isFlickerEnabled returns true when GOLI_TUI_DEBUG=1', async () => {
    const { isFlickerEnabled } = await import('../src/tui/lib/flickerStore.js');
  ...
  it('recordFlickerFrame increments totalFlickerFrames and updates lastFlickerAt', async () => {
    const { recordFlickerFrame, getFlickerSnapshot } = await import('../src/tui/lib/flickerStore.js');
  ...
  it('subscribeFlicker receives state updates via microtask', async () => {
    const { recordFlickerFrame, subscribeFlicker } = await import('../src/tui/lib/flickerStore.js');
    const events: number[] = [];
  ...
  it('onFlicker fires callback synchronously on each flicker', async () => {
    const { recordFlickerFrame, onFlicker } = await import('../src/tui/lib/flickerStore.js');
    let calls = 0;
  ...
  it('resetFlickerState zeroes the counters', async () => {
    const { recordFlickerFrame, resetFlickerState, getFlickerSnapshot } = await import('../src/tui/lib/flickerStore.js');
    recordFlickerFrame();
    recordFlickerFrame();
    resetFlickerState();
  ...
  it('subscribeFlicker is a no-op when disabled', async () => {
    delete process.env['GOLI_TUI_DEBUG'];
    vi.resetModules();
    const { subscribeFlicker, recordFlickerFrame } = await import('../src/tui/lib/flickerStore.js');
    const events: number[] = [];
    const unsub = subscribeFlicker((s) => events.push(s.totalFlickerFrames));
    recordFlickerFrame();
...
describe('T-060: debugProfiler', () => {
  ...
  it('reportAction pushes to actionTimestamps (debounced at 16ms)', async () => {
    const { profiler } = await import('../src/tui/lib/debugProfiler.js');
    profiler.reset();
  ...
  it('wireFlickerToProfiler connects flickerStore to profiler.reportFlicker', async () => {
    const flicker = await import('../src/tui/lib/flickerStore.js');
    const { profiler, wireFlickerToProfiler } = await import('../src/tui/lib/debugProfiler.js');
    profiler.reset();
...
describe('T-060: DebugProfiler component', () => {
  ...
  it('MaybeDebugProfiler renders nothing visible when when=false', async () => {
    const { MaybeDebugProfiler } = await import('../src/tui/components/DebugProfiler.js');
  ...
  it('DebugProfiler renders stats when enabled', async () => {
    const { DebugProfiler } = await import('../src/tui/components/DebugProfiler.js');
    const { profiler } = await import('../src/tui/lib/debugProfiler.js');
    profiler.reset();
    profiler.profilersActive = 1;
...
describe('T-060: batchedScroll', () => {
  ...
  it('scheduleScrollTop is a no-op when no setter is registered', async () => {
    const { scheduleScrollTop, resetBatchedScroll } = await import('../src/tui/lib/batchedScroll.js');
    resetBatchedScroll();
    expect(() => scheduleScrollTop(100)).not.toThrow();
  });

  it('scheduleScrollTop coalesces multiple calls into one setter call', async () => {
    const { scheduleScrollTop, registerScrollSetter, resetBatchedScroll } = await import('../src/tui/lib/batchedScroll.js');
    resetBatchedScroll();
    const calls: number[] = [];
  ...
  it('flushPendingScroll applies the pending value immediately', async () => {
    const { scheduleScrollTop, flushPendingScroll, registerScrollSetter, resetBatchedScroll } = await import('../src/tui/lib/batchedScroll.js');
    resetBatchedScroll();
    const calls: number[] = [];
    const unsub = registerScrollSetter((v) => calls.push(v));
  ...
  it('getPendingScrollTop returns the latest scheduled value', async () => {
    const { scheduleScrollTop, getPendingScrollTop, registerScrollSetter, resetBatchedScroll } = await import('../src/tui/lib/batchedScroll.js');
    resetBatchedScroll();
...
describe('T-060: useFlickerDetector hook', () => {
  ...
  it('useFlickerDetector is importable and callable', async () => {
    const mod = await import('../src/tui/hooks/useFlickerDetector.js');
    expect(typeof mod.useFlickerDetector).toBe('function');
  });

  it('a component using the hook renders without crashing', async () => {
    const { useFlickerDetector } = await import('../src/tui/hooks/useFlickerDetector.js');
