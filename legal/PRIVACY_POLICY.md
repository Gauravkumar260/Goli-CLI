...
the configured model provider (default: Z.ai for GLM-5.2):
...
### 4.1 Sandboxing
All command execution is sandboxed (bubblewrap on Linux, sandbox-exec
on macOS). The sandbox prevents the agent from:
- Writing outside the workspace.
- Accessing the network except via an allowlist.
...
- The `GOLI_MODEL_API_KEY` environment variable, OR
- The `config/default.toml` file (with `0600` permissions recommended).

GOLI-CLI does NOT store API keys in any other location.

## 5. User Choices

### 5.1 Choosing a Model Provider
Users can choose between:
- **Hosted API** (Z.ai, DeepSeek, Together AI, Moonshot) — data is
  sent to the provider's servers.
- **Self-hosted vLLM** — data never leaves the user's infrastructure.

See `infra/README.md` for self-hosting instructions.
...
- **Open-weight model:** GLM-5.2's weights are auditable.
- **Audit log:** Every tool call is logged for accountability.
- **Human oversight:** The approval engine requires human approval
  for Tier-2+ actions.
- **Risk management:** The blast-radius calculator (`approval/blast-radius.ts`)
...
`TERMS_OF_SERVICE.md` and `SECURITY.md`.*
