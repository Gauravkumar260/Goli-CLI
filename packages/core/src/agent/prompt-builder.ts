...
export class PromptBuilder {
  ...
  assemble(ctx: PromptBuildContext): AssembledPrompt {
    const fromCache = this.cacheValid;

    // Build or reuse stable tier
    if (!this.cacheValid || this.cachedStable === null) {
   ...
   */
  isCacheValidFor(_ctx: PromptBuildContext): boolean {
    if (!this.cacheValid) return false;
    // Check if model or provider changed (requires rebuild)
    // The actual check compares against what was cached
    return true;
  }
