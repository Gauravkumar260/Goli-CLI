...
export async function runStreamingCompletion(params: {
  ...
  call: (opts: { stream: boolean }) => Promise<ModelResponse>;
  ...
  try {
    // The call function should invoke streamCallback for each delta
    // and null at the end. We intercept via a wrapper.
    ...
    void wrappedCallback; // The call function receives this via closure

    const response = await call({ stream: true });
