 ...
 *
 * ## Locale fallback (MEDIUM-83)
 *
 * The previous implementation only special-cased `zh` → `zh-CN`. A
 * user with `GOLI_LANG=es-MX` (Mexican Spanish) got English instead
 * of Spanish, because the fallback logic didn't try the language
 * prefix. We now try the language prefix for ALL locales: `es-MX` →
 * `es`, `de-AT` → `de`, `ja-JP` → `ja`, `pt-BR` → (no match → en),
 * etc.
 */
export function initI18n(env: NodeJS.ProcessEnv = process.env): void {
  ...
  // Try the language prefix (e.g., "es-MX" → "es", "zh-TW" → "zh").
  // Normalize: lowercase the prefix for case-insensitive matching.
  const langPart = requested.split(/[-_]/)[0]?.toLowerCase();
  if (langPart) {
    // Try each supported locale's language prefix.
    for (const supported of SUPPORTED_LOCALES) {
      const supportedLang = supported.split(/[-_]/)[0]?.toLowerCase();
      if (supportedLang === langPart) {
