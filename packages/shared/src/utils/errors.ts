...
export abstract class GoliError extends Error {
   ...
   * ## `context` redaction
   *
   * The previous implementation included the `context` field (passed to
   * the constructor for structured logging) verbatim in `toJSON()`. The
   * `context` field can contain user-supplied data, command arguments,
   * request bodies, or other secrets — exposing it in the JSON form
   * (which the audit log and OTel spans consume) risks persisting
   * secrets to disk. We now EXCLUDE `context` from `toJSON()` by
   * default. Callers that explicitly want to log context should set
   * `includeContext: true` (a new opt-in flag).
   */
  toJSON(): Record<string, unknown> {
    const own: Record<string, unknown> = {};
    // Collect subclass-specific fields (status, toolName, mode, etc.)
    // EXCLUDE `context` — it may contain secrets (HIGH-85).
    for (const key of Object.getOwnPropertyNames(this)) {
      if (key === 'message' || key === 'stack' || key === 'cause' || key === 'context') continue;
