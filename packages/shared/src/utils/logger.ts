 ...
 * process startup.
 *
 * ## Resource cleanup (MEDIUM-87)
 *
 * The previous implementation did NOT close the OLD logger's
 * lifecycle stream before replacing it. Each `configureLogger` call
 * leaked a file descriptor (the open WriteStream on the previous
 * lifecycle log). We now call `close()` on the previous logger
 * before replacing it.
 *
 * @param opts
 */
export function configureLogger(opts: LoggerOptions): void {
  // Close the previous logger's lifecycle stream to avoid FD leaks.
  try {
    logger.close?.();
  } catch {
    // Best-effort — don't crash on a failing close.
  }
...
export function createLogger(opts: LoggerOptions = {}): Logger {
  ...
  // Use a single mutable variable so emit() and close() share state.
  // The previous implementation had TWO variables (`lifecycleStream`
  // const + `_lifecycleStream` let), and emit() read the const while
  // close() null'd the let — so emit() kept writing to the closed
  // stream after close() (MEDIUM-88).
  let lifecycleStreamRef: LogStream | null = lifecycleStream;
  ...
  function emit(level: LogLevel, msg: string, context: LogContext): void {
    ...
    stream.write(line + '\n');
    if (lifecycleStreamRef) {
      // Lifecycle log is always JSON for machine parsing (TUI `parentLog`).
      // The previous implementation wrote `[goli] ${ts} ${level} ${msg}`
      // (non-JSON) despite the "always JSON" comment — TUI consumers
      // expecting JSON.parse-able lines broke on the bracket prefix.
      // We now write the same JSON line as the primary stream.
      lifecycleStreamRef.write(JSON.stringify(merged) + '\n');
    }
  }
  ...
  return {
    ...
    flush(): void {
      ...
      if (lifecycleStreamRef && typeof lifecycleStreamRef.write === 'function') {
        lifecycleStreamRef.write('');
      }
    },
    close(): void {
      // Close the lifecycle stream to free the file descriptor.
      // Set the ref to null FIRST so concurrent emit() calls (which
      // check the ref) skip the write — closing a stream that's
      // mid-write can throw EPIPE.
      const s = lifecycleStreamRef;
      lifecycleStreamRef = null;
      if (s) {
        const endable = s as unknown as { end?: () => void };
        if (typeof endable.end === 'function') {
          try { endable.end(); } catch { /* best-effort */ }
        }
      }
    },
  };
