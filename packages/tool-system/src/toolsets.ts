...
export function resolveToolset(toolsetName: string): string[] {
  ...
  // Recursively expand includes with cycle detection. The previous
  // implementation had no visited-set, so a future edit creating a
  // cycle (e.g., toolset A includes B, B includes A) would recurse
  // until the call stack overflowed (`RangeError: Maximum call stack
  // size exceeded`). The current TOOLSETS map has no cycles, but
  // the API doesn't prevent them — a future config-driven toolset
  // loader could introduce one. We now track visited toolset names.
  const expandIncludes = (ts: Toolset, visited: Set<string>): void => {
    if (!ts.includes) return;
    for (const include of ts.includes) {
      if (visited.has(include)) {
        // Cycle — break it. Don't throw (the agent may be in the
        // middle of a turn and a throw here would crash it); just
        // log via console.warn and skip.
         
        console.warn(`[goli-cli] toolset cycle detected: ${include} (already visited: ${[...visited].join(' → ')})`);
        continue;
      }
      ...
      visited.add(include);
      expandIncludes(included, visited);
      // Note: we do NOT remove `include` from `visited` after the
      // recursion returns — this is the simpler approach (treat
      // each toolset as visited-at-most-once per resolveToolset
      // call, which is the diamond case: A→B, A→C, B→D, C→D — D
      // is visited once via B, skipped via C, which is correct).
    }
  };

  expandIncludes(toolset, new Set([toolsetName]));
  return [...tools];
}
