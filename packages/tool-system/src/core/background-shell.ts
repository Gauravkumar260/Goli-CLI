...
export function startBackgroundShell(command: string, cwd: string): string {
  ...
  child.stdout?.on('data', (data: Buffer) => {
    ...
    // Use `substring` instead of `slice(-100_000)` — `slice`
    // on a string copies the entire 100KB on every data event
    // (O(n²) for chatty processes). `substring` with an explicit
    // start index avoids the negative-index copy.
    if (shell.stdout.length > 100_000) {
      shell.stdout = shell.stdout.substring(shell.stdout.length - 100_000);
    }
  });
  ...
  child.stderr?.on('data', (data: Buffer) => {
    ...
    if (shell.stderr.length > 100_000) {
      shell.stderr = shell.stderr.substring(shell.stderr.length - 100_000);
    }
  });
  ...
  child.on('exit', (code) => {
    shell.exited = true;
    shell.exitCode = code;
    // Auto-cleanup: remove the shell from the map 60 seconds
    // after exit so the `shells` Map doesn't grow indefinitely.
    // The previous implementation only removed shells via an
    // explicit `kill_shell` call — a shell that exited
    // naturally stayed in the map forever with its 100KB
    // stdout + 100KB stderr buffers. Over a long session,
...
async function killShellHandler(
  ...
  // kill_shell is T1 (it terminates a process the user may still
  // want running — e.g. a dev server). In build mode with an
  // interactive approver wired, prompt BEFORE killing. godMode and
  // autoMode skip the gate.
  if (ctx.requestApproval && !ctx.godMode && !ctx.autoMode) {
    const approvalDecision = await ctx.requestApproval({
      toolCallId: ctx.toolCallId,
      toolName: 'kill_shell',
      tier: 'T1',
      description: `kill background shell ${shellId} (${shell.command.slice(0, 80)})`,
      args,
    ...
    if (!approvalDecision.approved) {
      return {
        ...
        ok: false,
        content: '',
        error: `kill_shell denied by user${approvalDecision.reason ? `: ${approvalDecision.reason}` : ''}.`,
      };
    }
  }
