  ...
  handler: (ctx: HookContext): PostToolUseHookResult => {
    ...
    if (ctx.toolName === 'bash') {
      ...
      const modifyingPatterns = [
        ...
        /(?:^|\s|&&|;)\s*mkdir\s/,
        /(?:^|\s|&&|;)\s*tee\s/,
        // Redirect to file — exclude `/dev/null` so read-only
        // commands like `grep foo file > /dev/null` don't trigger
        // a checkpoint. The previous implementation used a bare
        // `>` pattern which matched any redirect including
        // `> /dev/null`, creating unnecessary stash entries.
        /(?:^|\s|&&|;)\s*>\s*(?!\/dev\/null)\S/,
        /(?:^|\s|&&|;)\s*sed\s.*-i/,
        /(?:^|\s|&&|;)\s*npm\s+install/,
        /(?:^|\s|&&|;)\s*pip\s+install/,
    ...
    try {
      // Use `git stash create -u` to include untracked files.
      // The previous implementation used `git stash create`
      // (without `-u`), which stashed tracked changes only. If
      // the bash command created untracked files (e.g., `touch
      // new-file.txt`), those were NOT in the stash. Restoring
      // the stash wouldn't remove the new untracked files. The
      ...
      if (ref) {
        ...
        try {
          ...
          // `git stash store` failed — the ref is dangling and
          // will be GC'd. The previous implementation silently
          // ignored this, so the user thought a checkpoint
          // existed but it didn't. We now log a warning.
          ctx.logger?.warn('git stash store failed — checkpoint not persisted', {
