...
export async function extractChunksWithTreeSitter(
  ...
  try {
    ...
    // The previous implementation silently returned `[]` on any
    // error — tree-sitter OOM, segfault, binding errors were all
    // invisible. The caller (indexFileAsync) fell back to regex,
    // but the original error was lost. We now log the error so
    // the user can diagnose tree-sitter issues.
     
    console.warn('[goli-cli] tree-sitter extraction failed, falling back to regex:', {
      filePath,
      language,
 ...
 *
 * Includes a depth limit (MAX_DEPTH = 200) to prevent stack overflow
 * on pathological trees. Tree-sitter ASTs are typically shallow, but
 * generated/serialized code can be deep.
 * @param node
 * @param visitor
 * @param depth
 */
const MAX_WALK_DEPTH = 200;
function walkTree(node: TreeNode, visitor: (node: TreeNode) => void, depth: number = 0): void {
  ...
  if (node.children) {
    ...
    for (let i = 0; i < node.childCount; i++) {
      ...
      if (child) walkTree(child, visitor, depth + 1);
    }
  }
}
