...
export class TreeSitterIndexer {
  ...
  indexFile(filePath: string): SemanticChunk[] {
    // TOCTOU fix: the previous implementation called
    // `existsSync(filePath)` then `readFileSync(filePath)`.
    // If the file was deleted between the check and the read,
    // `readFileSync` threw ENOENT which propagated to the
    // caller uncaught. We now just try to read and catch ENOENT.
    let content: string;
    try {
  ...
  async indexFileAsync(filePath: string): Promise<SemanticChunk[]> {
    // TOCTOU fix — see indexFile() for rationale.
    let content: string;
    try {
      content = readFileSync(filePath, 'utf-8');
    } catch (err) {
