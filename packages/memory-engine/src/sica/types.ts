 ...
 * - **LLM-based overseer**: separate model instance with veto power.
...
export interface ArchiveEntry {
  ...
  proposalId?: string;
  /**
   * Whether this version was adopted, reverted, a pre-cycle snapshot,
   * or the original version before any SICA changes.
   *
   * - `'adopted'`: the proposal was applied and is now live.
   * - `'reverted'`: the proposal was applied then rolled back.
   * - `'initial'`: the ORIGINAL version before any SICA changes
   *   (only the very first archive entry for a target should have
   *   this status). The previous implementation used `'initial'` for
   *   EVERY cycle's pre-cycle snapshot, polluting the archive and
   *   making `getLastAdopted` return non-adopted snapshots — we
   *   now use `'snapshot'` for those.
   * - `'snapshot'`: a pre-cycle snapshot of the current state,
   *   taken BEFORE applying the proposal. Used as the rollback
   *   target if the proposal is later reverted.
   */
  status: 'adopted' | 'reverted' | 'initial' | 'snapshot';
