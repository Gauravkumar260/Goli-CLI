...
program
	...
	.action(async (options) => {
		...
		if (options.split === "held-out") {
			...
			console.log(
				"This should only be done for final phase verification to prevent prompt overfitting.",
			);
		...
		const taskIds = JSON.parse(
			await fs.readFile(splitPath, "utf-8"),
		) as string[];
		...
		const runLimit = Number.parseInt(options.limit);
		...
		for (const task of tasksToRun) {
			try {
				...
				if (!record.grade.passed) {
					...
					console.log(
						`[${task.task_id}] ❌ FAILED | Mode: ${analysis.failure_class} | Evidence: ${analysis.evidence}`,
					);
				} else {
					successCount++;
					console.log(
						`[${task.task_id}] ✅ PASSED | Score: ${record.grade.score.toFixed(2)} | Reason: ${record.grade.reason}`,
					);
				}
		...
		console.log("\n──────────────────────────────────────────────────────────");
		console.log(
			`🏁 Eval Complete: ${successCount}/${tasksToRun.length} passed.`,
		);
