...
program
	...
	.action(async (options) => {
		...
		const GOLI_CLI_HOME =
			process.env.GOLI_CLI_HOME ||
			path.join(require("node:os").homedir(), ".goli_cli");
		...
		let agreed = 0;
		let count = 0;
		const targetCount = Number.parseInt(options.count);
		...
		for (const file of sessionFiles) {
			...
			console.log(
				`Model Verdict: ${gradeEvent.passed ? "✅ PASSED" : "❌ FAILED"}`,
			);
			console.log(
				`Model Reasoning: ${gradeEvent.reason || gradeEvent.grade?.reason}`,
			);
