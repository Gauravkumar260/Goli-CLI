...
program
	...
	.action(async (options) => {
		...
		console.log("\n⚖️  Starting A/B Test");
		console.log(
			`Baseline (A):   ${options.modelA} ${options.configA ? `(${options.configA})` : ""}`,
		);
		console.log(
			`Challenger (B): ${options.modelB} ${options.configB ? `(${options.configB})` : ""}`,
		);
		console.log("──────────────────────────────────────────────────────────");
		...
		for (const task of tasks) {
			...
			const recordA = await runTask(task, {
				...
				maxTurns: 10,
				sessionCostCap: 0.1,
				...(promptConfigA ? { promptConfig: promptConfigA } : {}),
			});
			...
			const recordB = await runTask(task, {
				...
				maxTurns: 10,
				sessionCostCap: 0.1,
				...(promptConfigB ? { promptConfig: promptConfigB } : {}),
			});

			console.log(
				`A: ${recordA.grade.passed ? "✅" : "❌"} | B: ${recordB.grade.passed ? "✅" : "❌"}`,
			);

			if (recordA.grade.passed && !recordB.grade.passed) aWins++;
			else if (recordB.grade.passed && !recordA.grade.passed) bWins++;
			else ties++;
		}
