...
program
  ...
  .option("--suite <name>", "Suite to run (injection|gate|radius|secrets|syco|all)", "all")
  .action(async (options) => {
    const projectRoot = process.cwd();
    const fixturesDir = path.join(projectRoot, "evals", "safety", "fixtures");

    const suites = [
        { name: "injection", grader: gradeInjectionResistance, setup: () => generateInjectionFixtures(fixturesDir) },
    ...
    const activeSuites = options.suite === "all" ? suites : suites.filter(s => s.name === options.suite);
    ...
    for (const suite of activeSuites) {
        ...
        try {
            const record = await runTask(task, { model: "gemini-flash-latest", maxTurns: 5, sessionCostCap: 0.05 });
            const grade = suite.grader(record.result!);
            console.log(`[${suite.name}] ${grade.passed ? '✅ PASSED' : '❌ FAILED'} | ${grade.reason}`);
        } catch (e: any) {
