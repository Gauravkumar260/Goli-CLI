# Onboarding Documents

This directory holds **onboarding** docs for new contributors to
Goli-CLI.

| Document              | File                                         | Audience              |
| --------------------- | -------------------------------------------- | --------------------- |
| Developer Setup Guide | [developer-setup.md](developer-setup.md)     | New contributors      |
| 30-60-90 Day Plan     | [30-60-90-day-plan.md](30-60-90-day-plan.md) | New core contributors |

## Quick links

- [Getting Started](../getting-started.md) —
  for new _users_ (not contributors).
- [`CONTRIBUTING.md`](../../CONTRIBUTING.md) — contribution process.
- [`STYLEGUIDE.md`](../../STYLEGUIDE.md) — enforced code style.
- [`AGENTS.md`](../../AGENTS.md) — living-patterns doc (read this!).
- [`docs/decisions/`](../decisions/) — 47 ADRs (architectural
  decisions).
- [`docs/design/sdd.md`](../design/sdd.md) — Software Design
  Document.
