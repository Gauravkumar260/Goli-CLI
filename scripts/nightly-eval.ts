...
async function main() {
    ...
    try {
        ...
        const retrievalOut = execSync("bun evals/run-retrieval.ts", { encoding: "utf8" });
        await fs.appendFile(reportPath, "RETRIEVAL EVAL:\n" + retrievalOut + "\n\n");
        
        console.log("2. Running Agent Eval (TRAIN split)...");
        const agentOut = execSync("bun evals/run-agent.ts --split train --limit 5", { encoding: "utf8" });
        await fs.appendFile(reportPath, "AGENT EVAL (TRAIN):\n" + agentOut + "\n\n");

        console.log("3. Running Safety Suite...");
        const safetyOut = execSync("bun evals/safety/run-safety.ts --suite all", { encoding: "utf8" });
