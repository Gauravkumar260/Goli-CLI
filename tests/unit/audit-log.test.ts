...
import {
  ...
  verifyAuditLog,
  getAuditLogSummary,
} from '../../packages/core/src/sandbox/audit-log.js';

import type { AuditLogEntry } from '../../packages/core/src/sandbox/types.js';
...
describe('audit log', () => {
  ...
  it('appendAuditLog writes entries', () => {
  ...
  it('readAuditLog handles malformed entries gracefully', () => {
  ...
  it('verifyAuditLog returns ok for empty log', () => {
    const result = verifyAuditLog();
    expect(result.ok).toBe(true);
    expect(result.entryCount).toBe(0);
  });

  it('verifyAuditLog detects missing fields', () => {
    const logPath = getAuditLogPath();
  ...
  it('verifyAuditLog detects out-of-order timestamps', () => {
  ...
  it('getAuditLogSummary computes statistics', () => {
