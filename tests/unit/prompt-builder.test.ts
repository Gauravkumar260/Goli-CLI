...
import { PromptBuilder } from '../../packages/core/src/agent/prompt-builder.js';
import {
  applySystemAnd3Strategy,
  estimateTokenSavings,
  shouldCache,
} from '../../packages/core/src/agent/prompt-caching.js';
...
describe('PromptBuilder', () => {
  ...
  it('includes model line in volatile tier', () => {
    const result = builder.assemble(makeCtx({ model: 'glm-5.2', provider: 'vllm' }));
    expect(result.volatile).toContain('glm-5.2');
