...
import {
  PromptBuilder,
  computeStableHash,
} from '../../packages/core/src/agent/prompt-builder.js';
import {
  ToolsetSnapshot,
  computeToolNamesHash,
} from '../../packages/core/src/agent/toolset-snapshot.js';

import type { PromptBuildContext } from '../../packages/core/src/agent/prompt-builder.js';
...
describe('T-021: stableHash is stable across N turns (acceptance #3)', () => {
  ...
  it('stableHash changes when stable tier changes (model swap)', () => {
    ...
    const p1 = builder1.assemble(makeCtx({ model: 'glm-5.2' }));
    ...
    const p2 = builder2.assemble(makeCtx({ model: 'glm-5.2-flash' }));
...
describe('T-021: agent loop documents and enforces (acceptance #1)', () => {
  ...
  it('prompt-builder.ts documents the byte-stability invariant', async () => {
    ...
    const src = await readFile(
      resolve(process.cwd(), 'packages/core/src/agent/prompt-builder.ts'),
      'utf-8',
    );
